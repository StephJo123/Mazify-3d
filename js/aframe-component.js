...
AFRAME.registerComponent("trackballfinish", {
  tick: function () {
    ...
    if (
      Math.abs(pos.x - posSphere.x) < 2 &&
      Math.abs(pos.z - posSphere.z) < 2
    ) {
      clearInterval(mainCounter);
      $("finishDialog").children[0].children[1].children[0].innerHTML =
        "Félicitation, vous avez terminé le labyrinthe en " +
        Math.round(temps) +
        "s";
